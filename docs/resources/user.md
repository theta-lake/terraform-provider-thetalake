...
- `password` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) The user's password
- `password_confirmation` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Password confirmation, must match password
- `role_id` (Number) The ID of the role that will be assigned to the new user

### Optional

- `search_id` (Number) The ID of the search used for the user's security filter
...
- `has_datums` (Boolean) Indicates if the user is associated with any captured datums
...
- `archive_only` (Boolean) Indicates if the org unit is archive only
