...
func (c *Client) CreateLabel(ctx context.Context, label Label) (Label, error) {
	...
	err := c.doRequest(http.MethodPost, "/labels", label, "label", &responseLabel)
	if err != nil {
		return Label{}, err
	}
...
func (c *Client) GetLabelById(ctx context.Context, labelId int64) (Label, error) {
	...
	err := c.doRequest(http.MethodGet, endpoint, nil, "label", &responseLabel)
	if err != nil {
		return Label{}, err
	}
...
func (c *Client) UpdateLabel(ctx context.Context, label Label) (Label, error) {
	...
	err := c.doRequest(http.MethodPut, endpoint, label, "label", &responseLabel)
	if err != nil {
		return Label{}, err
	}
...
func (c *Client) DeleteLabel(ctx context.Context, labelId int64) error {
	endpoint := fmt.Sprintf("/labels/%v", labelId)
	err := c.doRequest(http.MethodDelete, endpoint, nil, "", nil)
	if err != nil {
		return err
	}
